        ...
        for result in results:
            for envelope in result.envelopes:
                if not envelope.verify():
                    raise ValueError(f"provider produced invalid envelope: {envelope.envelope_id}")
...
def doctor_live(registry: ProviderRegistry | None = None, runner: Any | None = None) -> dict[str, Any]:
    ...
    for command in unique:
        # Do not execute providers during doctor; resolve availability through a harmless runner probe abstraction.
        import shutil

        commands[command] = {"available": shutil.which(command) is not None, "path": shutil.which(command)}
    return {
